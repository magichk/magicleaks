...
def pwndb_main(email, session):
    passwords = []
    results = []
    leaks = pwndb_find_leaks(email, session)
    if leaks:
        for leak in leaks:
            results.append(leak)
    results.pop(0)
    for result in results:
        passwords.append(str(result))
    return passwords


def pwndb_find_leaks(email, session):
...
def pwndb_main_funct(email):
	...
	try:
		...
		print("Can't connect to service! restart tor service and try again.")
...
def pwndb(email):
	try:
		passwords = pwndb_main_funct(email)
	except:
		print("error")
	...
	if (check == 1):
		try:
			...
			if (result):
				...
				try:
					print("\033[1;33mChecking leaks for email account " + email + " on tor...")
					print("\033[1;33m--------------------------------------------------------")
					passwords = pwndb(email)

					if not passwords:
						print ("\033[1;32mNo leaks found for "+email)
					for i in passwords:
						print ("\033[1;37mThis is a passwords leakeds for this email account: \033[1;31m" + str(i))
				except:
					print ("\033[1;31mPosiblemente tor no funciona y por eso pwndb no puede ser consultada")
		except:
